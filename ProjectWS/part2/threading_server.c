...
char *image_1;
int image_1_size;
//struct client_throughput_ *client_list;
List* client_list;
...
typedef struct client_throughput_ {
	int clientID;	// The ID of the client, as generated by the server.
	int througput;	// The value of the throughput number. used to decide on how much to slow down.
//	struct client_throughput_ *next;
}client_throughput;
...
//  generate_echo_response(buffer, response);
	...
	while (client_args.close_requested == 0
			|| image_buffer_send_offset < image_1_size) {
...
int main(int argc, char **argv) {
	...
	while (true) {
		...
		char sendStr[24];
		sprintf(sendStr, "%s%d", "clientID:", clientID);

		// Send the client the ID, so it can know what ID to send along with the throughput.
		fprintf(stderr,"Send a clientID: %s", sendStr);
		int bytes_written = send(client, sendStr, sizeof(sendStr), 0);
		fprintf(stderr, "DONE send a clientID: %s", sendStr);

		// Add the new ID to the list.

		client_throughput *client_t = (client_throughput *) malloc(
				sizeof(client_throughput));
		client_t->clientID = clientID;
		client_t->througput = -1;

		list_append(client_list, client_t);

		fprintf(stderr, "List is now this long : %d", list_length(client_list));
